...
- [server/src/routes/purchaseOrderRoutes.js](file://server/src/routes/purchaseOrderRoutes.js)
...
## 更新摘要
**变更内容**
- 新增供应商文档管理子模块，整合采购订单、发票、退货管理的完整业务流程
- 新增采购订单路由和控制器，支持PO创建、编辑、删除和附件管理
- 新增供应商发票路由和控制器，支持发票创建、编辑、删除和附件管理
- 新增供应商退货路由和控制器，支持退货/索赔/维修文档管理
- 新增共享附件上传工具，统一处理供应商文档附件
- 新增数据库迁移文件，定义供应商文档相关表结构
- 新增前端页面和组件，提供完整的用户界面
...
## 引言
本文件面向库存管理系统的核心功能模块，围绕以下主题展开：用户认证与权限控制（JWT、角色授权、会话管理）、商品与分类管理、供应商管理、库存管理（查询、出入库、调拨、分配/释放、盘点）、仓库管理、电商集成（平台连接、商品同步、订单同步）、报表分析（库存与流水）、通知与告警中心、**供应商文档管理**（采购订单、发票、退货管理）。文档在技术深度与可读性之间取得平衡，既提供代码级分析，也给出可视化图示、流程图与最佳实践建议。
...
SUPPLIER_DOCS["供应商文档页面<br/>采购订单/发票/退货管理"]
END
subgraph "后端(Server)"
EXPRESS["Express 应用"]
AUTH_MW["认证中间件<br/>JWT 校验/角色授权"]
ROUTES["各模块路由<br/>auth/master/inventory/supplier-docs/..."]
UTILS["工具模块<br/>inventoryService/pagination/costAccess"]
DB["PostgreSQL 连接池"]
ATTACH_UTILS["附件上传工具<br/>统一文件处理"]
END
VUE_APP --> EXPRESS
EXPRESS --> AUTH_MW
AUTH_MW --> ROUTES
...
- **供应商文档管理**：采购订单（PO）、供应商发票、退货/索赔/维修文档的完整生命周期管理。
...
- [server/src/routes/purchaseOrderRoutes.js:19-395](file://server/src/routes/purchaseOrderRoutes.js#L19-L395)
- [server/src/routes/supplierInvoiceRoutes.js:52-416](file://server/src/routes/supplierInvoiceRoutes.js#L52-L416)
- [server/src/routes/supplierReturnRoutes.js:21-384](file://server/src/routes/supplierReturnRoutes.js#L21-L384)
...
## 架构总览
后端启动时进行数据库连通性校验，随后挂载统一中间件与路由。认证中间件对所有受保护路由生效，角色授权中间件限制特定操作的执行者范围。库存相关接口通过工具模块确保事务一致性与库存行存在性。**新增的供应商文档管理模块采用统一的附件上传工具，支持PDF、JPG、PNG、WebP格式，最大25MB限制**。
...
- [server/src/middleware/auth.js:5-29](file://server/src/middleware/auth.js#L5-L29)
- [server/src/utils/inventoryService.js:1-45](file://server/src/utils/inventoryService.js#L1-L45)
- [server/src/routes/purchaseOrderRoutes.js:308-395](file://server/src/routes/purchaseOrderRoutes.js#L308-L395)
- [server/src/routes/_supplierDocAttachment.js:33-84](file://server/src/routes/_supplierDocAttachment.js#L33-L84)
...
**更新** 新增供应商文档管理子模块，整合采购订单、发票、退货管理的完整业务流程

供应商文档管理模块提供完整的供应商文档生命周期管理，包括采购订单（PO）、供应商发票、退货/索赔/维修文档的创建、编辑、删除和附件管理。

#### 采购订单管理
- PO 创建：支持供应商选择、PO编号、日期、备注和明细行管理
- PO 编辑：支持供应商变更、PO号重复校验、事务性更新
- PO 删除：支持级联删除，自动清理附件
- 附件管理：支持PDF、JPG、PNG、WebP格式，最大25MB限制

#### 供应商发票管理
- 发票创建：必须关联有效采购订单，自动计算金额和数量
- 发票编辑：支持PO关联校验，事务性更新
- 发票删除：支持删除，自动清理附件
- 金额计算：自动计算每行金额（数量×单价-折扣），总计汇总

#### 供应商退货管理
- 文档类型：支持RETURN（退货）、CLAIM（索赔）、REPAIR（维修）
- 退货创建：支持供应商选择、文档类型、文档编号、日期和明细
- 退货编辑：支持文档类型校验，事务性更新
- 退货删除：支持删除，自动清理附件

```mermaid
flowchart TD
SupplierDocs["供应商文档管理"] --> PO["采购订单管理"]
SupplierDocs --> Invoice["供应商发票管理"]
SupplierDocs --> Return["供应商退货管理"]
PO --> POCreate["创建PO<br/>供应商+PO号+日期+明细"]
PO --> POEdit["编辑PO<br/>事务性更新"]
PO --> PODel["删除PO<br/>级联清理"]
Invoice --> InvCreate["创建发票<br/>关联PO+计算金额"]
Invoice --> InvEdit["编辑发票<br/>PO校验+金额重算"]
Invoice --> InvDel["删除发票<br/>附件清理"]
Return --> RetCreate["创建退货<br/>类型校验+明细"]
Return --> RetEdit["编辑退货<br/>类型校验+事务更新"]
Return --> RetDel["删除退货<br/>附件清理"]
POCreate --> POAttach["附件上传<br/>PDF/JPG/PNG/WebP"]
InvCreate --> InvAttach["附件上传<br/>PDF/JPG/PNG/WebP"]
RetCreate --> RetAttach["附件上传<br/>PDF/JPG/PNG/WebP"]
```
...
- **附件处理优化**：供应商文档附件采用统一上传工具，支持文件类型校验和大小限制，避免无效文件占用存储空间。
...
- **供应商文档上传失败**
  - 症状：附件上传报错或文件过大。
  - 排查：确认文件类型为PDF/JPG/PNG/WebP；检查文件大小不超过25MB；验证父文档是否存在；查看服务器磁盘空间。
...
- [server/src/routes/purchaseOrderRoutes.js:381-392](file://server/src/routes/purchaseOrderRoutes.js#L381-L392)
- [server/src/routes/supplierInvoiceRoutes.js:402-413](file://server/src/routes/supplierInvoiceRoutes.js#L402-L413)
- [server/src/routes/supplierReturnRoutes.js:370-381](file://server/src/routes/supplierReturnRoutes.js#L370-L381)

## 结论
本系统以模块化路由与中间件为核心，围绕库存全生命周期管理构建了从商品、供应商、仓库到电商集成与报表分析的完整能力。**新增的供应商文档管理模块整合了采购订单、发票、退货管理的完整业务流程，采用统一的附件处理机制，支持多种文件格式和严格的大小限制**。通过 JWT 与角色授权保障安全，通过事务与工具模块确保库存一致性，通过审计与日志完善可观测性。建议在生产环境中结合速率限制、监控告警与定期盘点，持续优化性能与稳定性。
...
- 使用示例与最佳实践
  ...
  - **供应商文档管理**：使用统一的供应商文档页面管理PO、发票、退货；注意附件格式和大小限制；定期清理过期文档。