    ...
    <div class="my-8 w-full max-w-5xl rounded-lg bg-white shadow-xl">
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-3">
        <h3 class="text-lg font-semibold text-slate-800">
          {{ form.id ? `Purchase Order #${form.po_no}` : 'New Purchase Order' }}
        </h3>
      ...
      <form class="space-y-4 px-5 py-4" @submit.prevent="submit">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div>
            ...
            <input v-model="form.po_date" required type="date" class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label class="block text-xs font-medium text-slate-600">PO No <span class="text-red-500">*</span></label>
            <input v-model="form.po_no" required class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm" />
          </div>
        </div>
...
async function loadAttachments() {
  if (!form.id) return
  try {
    const { data } = await api.get(`/purchase-orders/${form.id}`)
...
async function loadExisting(id) {
  ...
  form.po_no = data.po_no
  form.po_date = String(data.po_date).slice(0, 10)
