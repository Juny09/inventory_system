...
function normalizeOcrReviewContext(rawContext = null, documentType = 'invoice') {
...
async function loadOcrConfirmationTimeline(entityId, tenantId) {
  ...
  return result.rows.map((row) => {
    ...
    const ocrReviewContext = normalizeOcrReviewContext(summary.ocrReviewContext, 'invoice')
    return {
      audit_id: row.id,
      action: row.action,
...
async function syncProductPricingFromInvoiceItems(client, { tenantId, items, userId, invoiceNo }) {
  ...
  for (const [productId, unitPrice] of unitPriceMap.entries()) {
    ...
    await client.query(
      ...
      `,
      [productId, tenantId, costChanged, newCost, resolvedMarkup],
    )
    ...
    await client.query(
      ...
      `,
      [productId, tenantId, resolvedMarkup, newCost],
    )
  }
}
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const ocrReviewContext = normalizeOcrReviewContext(ocr_review_context, 'invoice')
    req.auditContext = {
